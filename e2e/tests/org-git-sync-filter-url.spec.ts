...
test('org git sync filter URLs apply and copy links round-trip', async ({ page, context }) => {
  ...
  await page.goto('/settings/git?linked=linked&lastSync=failed');
  ...
  await page.goto('/settings/git?runSource=manual&runStatus=failed#org-git-sync-runs');
  ...
  await page.goto('/settings/git?linked=unlinked&runSource=scheduled');
  await expect(page.getByTestId('git-sync-page-filter-toolbar')).toBeVisible();
  await expect(page.getByTestId('git-sync-overview-active-filter-linked')).toBeVisible();
  await expect(page.getByTestId('org-git-sync-runs-active-filter-runSource')).toBeVisible();

  await page.goto('/settings/git?provider=github&enabled=enabled');
  await expect(page.getByTestId('git-sync-overview-copy-filter-link')).toBeVisible();
  ...
  expect(overviewLink).toContain('provider=github');
  expect(overviewLink).toContain('enabled=enabled');
  await page.goto(overviewLink);
  await expect(page.getByTestId('git-sync-overview-active-filter-provider')).toBeVisible();
  await expect(page.getByTestId('git-sync-overview-active-filter-enabled')).toBeVisible();

  await page.goto('/settings/git?linked=linked&runSource=webhook&runStatus=failed#org-git-sync-runs');
  await expect(page.getByTestId('git-sync-page-copy-filter-link')).toBeVisible();
  ...
  expect(pageLink).toContain('runSource=webhook');
