...
public class ProjectGitLinkController {
    ...
    @PostMapping("/api/v1/projects/{projectId}/git-link/regenerate-webhook-secret")
    @Operation(summary = "Regenerate git webhook HMAC secret for linked repository")
    public ProjectGitLinkResponse regenerateWebhookSecret(
            @PathVariable UUID projectId,
            @AuthenticationPrincipal AuthenticatedUser user
    ) {
        return gitSyncService.regenerateWebhookSecret(projectId, user.getId());
    }

    @DeleteMapping("/api/v1/projects/{projectId}/git-link")
    @ResponseStatus(HttpStatus.NO_CONTENT)
    @Operation(summary = "Disconnect git repository link")
    public void delete(
            @PathVariable UUID projectId,
            @AuthenticationPrincipal AuthenticatedUser user
    ) {
        gitSyncService.deleteLink(projectId, user.getId());
    }
