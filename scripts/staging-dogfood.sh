...
if [[ -n "${METRICS_SCRAPE_TOKEN:-}" ]]; then
  ...
  echo "==> 4/5 skipped — set METRICS_SCRAPE_TOKEN to verify internal Prometheus scrape"
fi

echo "==> 5/5 manual dogfood (operator)"
